...
export const BADGES: Record<string, { title: string; description: string; icon: typeof Award }> = {
  first_steps: { title: "First Steps", description: "Completed 10 attempts.", icon: Sparkles },
  first_mock: {
    title: "First Mock Done",
    description: "Finished your first full mock exam.",
    icon: ClipboardCheck,
  },
  seven_day_streak: {
    title: "7-Day Streak",
  ...
  level_up: {
    title: "Level Up",
    description: "Scored correctly above your starting level.",
    icon: TrendingUp,
  },
  century_club: { title: "Century Club", description: "Logged 100 attempts.", icon: Trophy },
};

export function BadgesShelf({ earned, className }: { earned: Set<string>; className?: string }) {
      ...
      {all.map(([code, b]) => {
        ...
        return (
          ...
          >
            ...
            {!got && (
              <div className="text-[10px] mt-1 uppercase tracking-wide text-muted-foreground">
                Locked
              </div>
            )}
          </div>
